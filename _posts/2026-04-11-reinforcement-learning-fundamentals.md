...
tags:
  ...
  - deep learning
excerpt: "智能体、环境与策略；马尔可夫性质与价值函数；动态规划、MC、TD、Q-learning 与 SARSA；DQN、策略梯度、Actor-Critic、SAC、PPO 与 MCTS 要点整理。"
---
...
强化学习研究**智能体**通过与**环境**反复交互、根据反馈调整行为，从而完成目标。核心循环：在状态 \\(s\\) 下，智能体依据策略 \\(\pi\\) 选择动作 \\(a\\)，环境给出奖励 \\(r\\) 与下一状态 \\(s'\\)；奖励信号驱动策略改进，如此迭代。
...
| 状态 \\(s\\) | 环境描述 |
| 动作 \\(a\\) | 智能体决策 |
| 策略 \\(\pi(a\|s) = P(A_t=a \mid S_t=s)\\) | 在状态 \\(s\\) 下选动作 \\(a\\) 的概率（随机策略） |
| 奖励 \\(R_{t+1}\\) | 在 \\(t\\) 时刻于状态 \\(S_t\\) 采取 \\(A_t\\) 后，在 \\(t+1\\) 时刻观测到的标量反馈 |
| 转移 \\(P_{ss'}^a\\) | 在 \\(s\\) 执行 \\(a\\) 后转移到 \\(s'\\) 的概率（模型的一部分） |

**状态价值** \\(v_\pi(s)\\)：从状态 \\(s\\) 出发、之后一直遵循策略 \\(\pi\\)，所能获得的折扣回报期望。

**环境动态（模型）**：若已知 \\(P(s', r \mid s, a)\\)，可用于规划；否则为无模型设定。
...
- **Model-based**：显式或学习环境的转移/奖励模型 \\(P(s', r \mid s, a)\\)，可做规划（如动态规划）。
...
- **策略迭代**：交替 **策略评估**（用贝尔曼方程算 \\(v_\pi\\)）与 **策略改进**（对 \\(v_\pi\\) 贪心），收敛到 \\(v^*, \pi^*\\)。
- **价值迭代**：直接迭代贝尔曼**最优**算子更新 \\(V^*\\)，再从中导出贪心策略 \\(\pi^*\\)。
...
直观理解：用 **目标** \\(r + \gamma \max_{a'} Q(s',a')\\) 拉向当前估计 \\(Q(s,a)\\)，步长由 \\(\alpha\\) 控制。
...
状态/动作空间很大时，表格 \\(Q(s,a)\\) 不可行，用神经网络逼近 \\(Q(s,a;\theta)\\) 或策略 \\(\pi(a|s;\phi)\\)。

### DQN

- **函数逼近**：\\(Q(s,a;\theta) \approx Q^*(s,a)\\)。
- **经验回放**：存储 \\((s,a,r,s')\\)，随机小批量训练，减弱样本相关性。
...
**目标**：直接参数化 \\(\pi_\theta(a|s)\\)，用策略梯度上升期望回报。
...
其中 \\(H(\pi(\cdot|s)) = -\mathbb{E}_{a\sim\pi}[\log \pi(a|s)]\\)，\\(\alpha\\) 权衡奖励与探索。
...
在**同一条轨迹数据**上做多轮小批量更新，用 **概率比** \\(r_t(\theta)=\frac{\pi_\theta(a_t|s_t)}{\pi_{\theta_{\text{old}}}(a_t|s_t)}\\) 的**裁剪**限制策略变化幅度，稳定训练。常配合 **GAE** 估计优势。
